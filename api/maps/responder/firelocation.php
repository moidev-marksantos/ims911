...
try {
	...
	WHERE crm.deleted = 0 AND loc.createddatetime != '' AND res.responder_tks_respondertype = 'Fire Department';", array());
    ...
    for ($x = 0; $x < $adb->num_rows($responder); $x++) {    
		...
		//if($adb->query_result($responder,$x,"responder_tks_respondertype") == 'Fire Department'){
        array_push(
            $result->features,
            array(
                "type" => "Feature",
