...
try {
	...
	WHERE crm.deleted = 0 AND loc.createddatetime != '' AND res.responder_tks_respondertype = 'Law Enforcement Department' ", array());
	
	//AND loc.createddatetime BETWEEN (DATE_SUB(NOW(),INTERVAL 30 MINUTE)) AND NOW();
    ...
    for ($x = 0; $x < $adb->num_rows($responder); $x++) {    
		...
		//if($adb->query_result($responder,$x,"responder_tks_respondertype") == 'Local Enforcement Division (LED)'){
        array_push(
            $result->features,
            array(
                "type" => "Feature",
