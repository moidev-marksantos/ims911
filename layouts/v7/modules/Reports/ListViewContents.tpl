							...
							<tr class="searchRow listViewSearchContainer">
                                                            <th class="inline-search-btn">
                                                                <div class="table-actions">
                                                                    ...
                                                                    <button class="searchAndClearButton btn-sm  btn btn-danger {if count($SEARCH_DETAILS) eq 0}hide{/if}" data-trigger="clearListSearch"><i class="ti-close"></i>&nbsp;{vtranslate("LBL_CLEAR",$MODULE)}</button>
                                                                </div>
                                                            </th>
								{foreach item=LISTVIEW_HEADER key=LISTVIEW_HEADER_KEY from=$LISTVIEW_HEADERS}
									<th>
											...
											<span class="value textOverflowEllipsis">
												{if $LISTVIEW_HEADERNAME eq 'reporttype'}
													{if $LISTVIEW_ENTRY_VALUE eq 'summary' || $LISTVIEW_ENTRY_VALUE eq 'tabular'}
														<center title="{vtranslate('LBL_DETAIL_REPORT', $MODULE)}"><i class="ti-list"></i>&nbsp;List Report</center>
													{elseif $LISTVIEW_ENTRY_VALUE eq 'chart'}
														<center title="{vtranslate('LBL_CHART_REPORT', $MODULE)}"><i class="ti-bar-chart"></i>&nbsp;Chart Report</center>
													{/if}
