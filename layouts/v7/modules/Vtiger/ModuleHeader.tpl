...
{strip}
	<div class="col-sm-11 col-xs-10 padding0 module-action-bar clearfix coloredBorderTop" style="padding-left: 0px;">
		<div class="module-action-content clearfix {$MODULE}-module-action-content" style="background-color: #032e61; color: #fff;">
			<div class="col-lg-7 col-md-6 col-sm-5 col-xs-11 padding0 module-breadcrumb module-breadcrumb-{$smarty.request.view} transitionsAllHalfSecond">
				...
				<h4 class="module-title pull-left text-uppercase" style="color: #fff;">
				
				{if $smarty.request.view eq 'Map'} 
					MAP 
				{elseif $smarty.request.view eq 'CovidTracker'} 
					COVID TRACKER 
				{elseif $smarty.request.view eq 'IncidentMap'} 
					INCIDENT MAP 
				{elseif $smarty.request.view eq 'HeatMap'} 
					HEAT MAP
				{elseif $smarty.request.view eq 'FloodMonitoring'} 
					FLOOD MONITORING
				{else}
				{vtranslate($MODULE, $MODULE)} {/if}</h4>&nbsp;&nbsp;</a>
			...
			<div class="col-lg-5 col-md-6 col-sm-7 col-xs-1 padding0 pull-right">
				...
				{if $BASIC_ACTION->getLabel() == 'LBL_IMPORT'}
					...
					" id="{$MODULE}_basicAction_{Vtiger_Util_Helper::replaceSpaceWithUnderScores($BASIC_ACTION->getLabel())}" type="button" class="btn addButton btn-default module-buttons"
				...
				{else}
					...
					text-shadow: none;
					border-color: rgba(255,255,255,.2);
					" id="{$MODULE}_listView_basicAction_{Vtiger_Util_Helper::replaceSpaceWithUnderScores($BASIC_ACTION->getLabel())}" type="button" class="btn addButton btn-default module-buttons"
					{if stripos($BASIC_ACTION->getUrl(), 'javascript:')===0}
				onclick='{$BASIC_ACTION->getUrl()|substr:strlen("javascript:")};'
					{else}
										...
										<li>
											<div class="settingsIcon">
												<button type="button" class="btn btn-default module-buttons dropdown-toggle" data-toggle="dropdown" aria-expanded="false" title="{vtranslate('LBL_SETTINGS', $MODULE)}">
													<span class="ti-settings" aria-hidden="true"></span>&nbsp;{vtranslate('LBL_CUSTOMIZE', 'Reports')}&nbsp; <span class="caret"></span>
												</button>
