...
{strip}
    ...
    {if $smarty.request.view eq 'HeatMap'}         
	...
	{if vtranslate($PAGETITLE, $QUALIFIED_MODULE) neq 'Bayan911' }
			{if $USER_MODEL->get('roleid') neq 'H4'}
			<script>
				var audio = new Audio('alert.mp3');
				var rolename = "{$CURRENT_USER_MODEL->getUserRoleName()}";		
                ...
                function getnotification(){
                    $.get("api/notification/checkcounter.php?rolename="+rolename, function(data, status){						
                       if(data == '1'){
                        $.get("api/notification/get.php?rolename="+rolename, function(data, status){						
						    if(data.length != 0){
							audio.play();                            
							swal({
                                title: "Incident Alert!",
                                text:  "Please check the list for more info.",
                                icon:  "warning",
