...
class Calllogs extends Vtiger_CRMEntity {
	var $table_name = 'vtiger_calllogs';
	var $table_index= 'calllogsid';

	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('vtiger_calllogscf', 'calllogsid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('vtiger_crmentity', 'vtiger_calllogs', 'vtiger_calllogscf');
	...
	var $related_tables = Array( 
					'vtiger_calllogscf' => Array('calllogsid')
					);
	...
	var $tab_name_index = Array(
		...
		'vtiger_calllogs'   => 'calllogsid',
	    'vtiger_calllogscf' => 'calllogsid');
	...
	var $list_fields = Array (
		...
		'Calllogs No' => Array('calllogs', 'calllogsno'),
/*'Calllogs No'=> Array('calllogs', 'calllogsno'),*/
		'Assigned To' => Array('crmentity','smownerid')
	);
	var $list_fields_name = Array (
		/* Format: Field Label => fieldname */
		'Calllogs No' => 'calllogsno',
/*'Calllogs No'=> 'calllogsno',*/
		'Assigned To' => 'assigned_user_id'
	);

	// Make the field link to detail view
	var $list_link_field = 'calllogsno';
...
/*'Calllogs No'=> 'calllogsno',*/
	...
	var $popup_fields = Array ('calllogsno');

	// For Alphabetical search
	var $def_basicsearch_col = 'calllogsno';

	// Column value to use on detail view record text display
	var $def_detailview_recname = 'calllogsno';

	// Used when enabling/disabling the mandatory fields for the module.
	// Refers to vtiger_field.fieldname values.
	var $mandatory_fields = Array('calllogsno','assigned_user_id');

	var $default_order_by = 'calllogsno';
	...
	*/
	function vtlib_handler($moduleName, $eventType) {
		global $adb;
 		if($eventType == 'module.postinstall') {
			...
			Calllogs::checkWebServiceEntry();
			Calllogs::createUserFieldTable($moduleName);
			Calllogs::addInTabMenu($moduleName);
	...
	static function checkWebServiceEntry() {
		...
		$sql       =  "SELECT count(id) AS cnt FROM vtiger_ws_entity WHERE name = 'Calllogs'";
		...
		if($adb->num_rows($result) > 0)
		{
			...
			if($no == 0)
			{
				...
				$ws_entitySql = "INSERT INTO vtiger_ws_entity ( id, name, handler_path, handler_class, ismodule ) VALUES".
						  " (?, 'Calllogs','include/Webservices/VtigerModuleOperation.php', 'VtigerModuleOperation' , 1)";
