...
class Users extends CRMEntity {
	...
	function retrieve_leftpanelhide($user_name) {
		global $adb;
		$query = "SELECT leftpanelhide from vtiger_users where user_name=? AND deleted=0";
		$result  =$adb->pquery($query, array($user_name));
		$leftpanelhide = $adb->query_result($result,0,'leftpanelhide');
	...
	function saveentity($module, $fileid='') {
		...
		if(empty($this->column_fields['date_format'])) {
			$this->column_fields['date_format'] = 'mm-dd-yyyy';
		}
