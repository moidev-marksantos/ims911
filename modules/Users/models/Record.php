...
class Users_Record_Model extends Vtiger_Record_Model
{
	...
	{
		$db = PearDatabase::getInstance();
		$query = 'SELECT COUNT(*) usercount FROM vtiger_users WHERE deleted = 0';
		$result = $db->pquery($query, array());
		return $db->query_result($result, 0, 'usercount');
	}

	public function checkAuthenticationCode($userId, $authenticationcode)
	{
		$db = PearDatabase::getInstance();
		$query = 'SELECT 1 FROM vtiger_authentication WHERE user_id = ? and authenticationcode = ?';
		$result = $db->pquery($query, array($userId, $authenticationcode));
		if ($db->num_rows($result) != 0) {
			return true;
		}
		return false;
	...
	public function saveAuthenticationCode($userId, $authenticationcode)
	{
		$db = PearDatabase::getInstance();
		$insertQuery = "INSERT INTO vtiger_authentication (user_id, authenticationcode) VALUES (?, ?)";
		$db->pquery($insertQuery, array($userId, $authenticationcode));
	}

	public function deleteAuthenticationCode($userId, $authenticationcode)
	{
		$db = PearDatabase::getInstance();
		$insertQuery = "DELETE FROM vtiger_authentication WHERE user_id = ? AND authenticationcode = ?";
		$db->pquery($insertQuery, array($userId, $authenticationcode));
	}
	...
	{
		$db = PearDatabase::getInstance();
		$query = "SELECT email1 , email2 FROM vtiger_users WHERE id = ?";
		$result = $db->pquery($query, array($userid));
		$email1 = $db->query_result($result, 0, 'email1');
		if ($email1 == "") {
			return $db->query_result($result, 0, 'email2');
		}
		return $email1;
	}

	public function checkifhasauthentication($userid)
	{
