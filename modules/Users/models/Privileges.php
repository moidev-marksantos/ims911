...
class Users_Privileges_Model extends Users_Record_Model {
	...
	public static function getResponderCount() {
		
		$db = PearDatabase::getInstance();
		$query = 'SELECT COUNT(*) respondercount FROM vtiger_responder LEFT JOIN vtiger_crmentity
		ON vtiger_responder.responderid = vtiger_crmentity.crmid
		WHERE vtiger_crmentity.deleted = 0';
		$result = $db->pquery($query, array());
		return (int)$db->query_result($result, 0, 'respondercount');
	}

	public static function _getUserCount() {		
		$db = PearDatabase::getInstance();
		$query = 'SELECT COUNT(*) user FROM vtiger_users LEFT JOIN vtiger_crmentity
		ON vtiger_users.id = vtiger_crmentity.crmid
		WHERE vtiger_crmentity.deleted = 0';
		$result = $db->pquery($query, array());
		return (int)$db->query_result($result, 0, 'user');
	}
