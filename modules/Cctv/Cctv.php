...
class Cctv extends Vtiger_CRMEntity {
	var $table_name = 'vtiger_cctv';
	var $table_index= 'cctvid';

	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('vtiger_cctvcf', 'cctvid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('vtiger_crmentity', 'vtiger_cctv', 'vtiger_cctvcf');
...
/*'Cctv No'=> 'cctvno',*/
	...
	var $popup_fields = Array ('cctvno');

	// For Alphabetical search
	var $def_basicsearch_col = 'cctvno';

	// Column value to use on detail view record text display
	var $def_detailview_recname = 'cctvno';

	// Used when enabling/disabling the mandatory fields for the module.
	// Refers to vtiger_field.fieldname values.
	var $mandatory_fields = Array('cctvno','assigned_user_id');

	var $default_order_by = 'cctvno';
	...
	*/
	function vtlib_handler($moduleName, $eventType) {
		global $adb;
 		if($eventType == 'module.postinstall') {
			...
			Cctv::checkWebServiceEntry();
			Cctv::createUserFieldTable($moduleName);
			Cctv::addInTabMenu($moduleName);
	...
	static function checkWebServiceEntry() {
		...
		$sql       =  "SELECT count(id) AS cnt FROM vtiger_ws_entity WHERE name = 'Cctv'";
		...
		if($adb->num_rows($result) > 0)
		{
			...
			if($no == 0)
			{
				...
				$ws_entitySql = "INSERT INTO vtiger_ws_entity ( id, name, handler_path, handler_class, ismodule ) VALUES".
						  " (?, 'Cctv','include/Webservices/VtigerModuleOperation.php', 'VtigerModuleOperation' , 1)";
