...
class Agency extends Vtiger_CRMEntity {
	var $table_name = 'vtiger_agency';
	var $table_index= 'agencyid';

	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('vtiger_agencycf', 'agencyid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('vtiger_crmentity', 'vtiger_agency', 'vtiger_agencycf');
	...
	var $related_tables = Array( 
					'vtiger_agencycf' => Array('agencyid')
					);
	...
	var $tab_name_index = Array(
		...
		'vtiger_agency'   => 'agencyid',
	    'vtiger_agencycf' => 'agencyid');
	...
	var $list_fields = Array (
		...
		'Agency No' => Array('agency', 'agencyno'),
/*'Agency No'=> Array('agency', 'agencyno'),*/
...
/*'Agency No'=> 'agencyno',*/
	...
	var $popup_fields = Array ('agencyno');

	// For Alphabetical search
	var $def_basicsearch_col = 'agencyno';

	// Column value to use on detail view record text display
	var $def_detailview_recname = 'agencyno';

	// Used when enabling/disabling the mandatory fields for the module.
	// Refers to vtiger_field.fieldname values.
	var $mandatory_fields = Array('agencyno','assigned_user_id');

	var $default_order_by = 'agencyno';
	...
	*/
	function vtlib_handler($moduleName, $eventType) {
		global $adb;
 		if($eventType == 'module.postinstall') {
			...
			Agency::checkWebServiceEntry();
			Agency::createUserFieldTable($moduleName);
			Agency::addInTabMenu($moduleName);
	...
	static function checkWebServiceEntry() {
		...
		$sql       =  "SELECT count(id) AS cnt FROM vtiger_ws_entity WHERE name = 'Agency'";
		...
		if($adb->num_rows($result) > 0)
		{
			...
			if($no == 0)
			{
				...
				$ws_entitySql = "INSERT INTO vtiger_ws_entity ( id, name, handler_path, handler_class, ismodule ) VALUES".
						  " (?, 'Agency','include/Webservices/VtigerModuleOperation.php', 'VtigerModuleOperation' , 1)";
