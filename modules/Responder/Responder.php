...
class Responder extends Vtiger_CRMEntity {
	var $table_name = 'vtiger_responder';
	var $table_index= 'responderid';

	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('vtiger_respondercf', 'responderid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('vtiger_crmentity', 'vtiger_responder', 'vtiger_respondercf');
	...
	var $related_tables = Array( 
					'vtiger_respondercf' => Array('responderid')
					);
	...
	var $tab_name_index = Array(
	    ...
	    'vtiger_respondercf' => 'responderid');
	...
	var $list_fields = Array (
		...
		'Responder No' => Array('responder', 'responderno'),
/*'Responder No'=> Array('responder', 'responderno'),*/
		'Assigned To' => Array('crmentity','smownerid')
	);
	var $list_fields_name = Array (
		/* Format: Field Label => fieldname */
		'Responder No' => 'responderno',
/*'Responder No'=> 'responderno',*/
		'Assigned To' => 'assigned_user_id'
	);

	// Make the field link to detail view
	var $list_link_field = 'responderno';
...
/*'Responder No'=> 'responderno',*/
	...
	var $popup_fields = Array ('responderno');

	// For Alphabetical search
	var $def_basicsearch_col = 'responderno';

	// Column value to use on detail view record text display
	var $def_detailview_recname = 'responderno';

	// Used when enabling/disabling the mandatory fields for the module.
	// Refers to vtiger_field.fieldname values.
	var $mandatory_fields = Array('responderno','assigned_user_id');

	var $default_order_by = 'responderno';
	...
	*/
	function vtlib_handler($moduleName, $eventType) {
		global $adb;
 		if($eventType == 'module.postinstall') {
			...
			Responder::checkWebServiceEntry();
			Responder::createUserFieldTable($moduleName);
			Responder::addInTabMenu($moduleName);
	...
	static function checkWebServiceEntry() {
		...
		$sql       =  "SELECT count(id) AS cnt FROM vtiger_ws_entity WHERE name = 'Responder'";
		...
		if($adb->num_rows($result) > 0)
		{
			...
			if($no == 0)
			{
				...
				$ws_entitySql = "INSERT INTO vtiger_ws_entity ( id, name, handler_path, handler_class, ismodule ) VALUES".
						  " (?, 'Responder','include/Webservices/VtigerModuleOperation.php', 'VtigerModuleOperation' , 1)";
