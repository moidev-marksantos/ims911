...
class Vtiger_Relation_Model extends Vtiger_Base_Model{
	...
	public function getListUrl($parentRecordModel) {
		//index.php?module=HelpDesk&amp;relatedModule=Documents&amp;view=Detail&amp;record=442&amp;mode=showRelatedList&amp;relationId=55&amp;tab_label=Documents&amp;app=MARKETING
		if($this->get('modulename') == 'Documents'){
			$url = 'module='.$this->getParentModuleModel()->get('name').'&relatedModule='.$this->get('modulename').
				'&view=Detail&record='.$parentRecordModel->getId().'&mode=showRelatedMedia&relationId='.$this->get('relation_id');
		}
		else{
			$url = 'module='.$this->getParentModuleModel()->get('name').'&relatedModule='.$this->get('modulename').
