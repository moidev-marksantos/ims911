...
class Vtiger_Module_Model extends Vtiger_Module
{
	...
	public function getNameFields()
	{
		...
		if ($nameFieldObject && $nameFieldObject->fieldname) {
			...
			switch ($moduleName) {
				case 'HelpDesk':
					$this->nameFields = array('ticket_title');
					$fieldNames = 'ticket_title';
					break;
				case 'Documents':
					$this->nameFields = array('notes_title');
	...
	public function getModuleBasicLinks()
	{
		...
		$getrespondercount = Users_Privileges_Model::getResponderCount();
		...
		if ($createPermission) {
			if ($moduleName === "Calendar") {
				...
				if ($moduleName == 'Responder') {
					if ($getrespondercount < $responder_limit) {
						$basicLinks[] = array(
							'linktype' => 'BASIC',
	...
	public function isModuleRelated($moduleName)
	{
		...
		if (in_array($moduleName, $relatedModules)) return true;
		return false;
	}
	...
	public function getAutoFillModule($moduleName)
	{
		...
		switch ($moduleName) {
			case 'Contacts':
				...
				break;
			case 'Project':
				$autoFillModules = array('Contacts', 'Accounts');
				break;
		}
	...
	public function getModuleIcon()
	{
		...
		if ($this->source == 'custom') {
			if ($title == 'Call Logs') {
				$moduleIcon = "<i style='font-size:20px;' class='ti-headphone-alt' title='$title'></i>";
			} else {
		...
		if ($lowerModuleName == 'task') {
			$moduleIcon = "<i style='font-size:20px;' class='ti-check-box' title='$title'></i>";
		}

		if ($title == 'Tickets') {
			$moduleIcon = "<i style='font-size:20px;' class='ti-ticket' title='$title'></i>";
		} else if ($title == 'Contacts') {
			$moduleIcon = "<i style='font-size:20px;' class='ti-user' title='$title'></i>";
		} else if ($title == 'Calendar') {
			$moduleIcon = "<i style='font-size:20px;' class='ti-calendar' title='$title'></i>";
		} else if ($title == 'Documents') {
			$moduleIcon = "<i style='font-size:20px;' class='ti-files' title='$title'></i>";
		} else if ($title == 'Emails') {
			$moduleIcon = "<i style='font-size:20px;' class='ti-email' title='$title'></i>";
		} else if ($title == 'ModComments') {
			$moduleIcon = "<i style='font-size:20px;' class='ti-comment-alt' title='$title'></i>";
		} else if ($title == 'CallLogs') {
			$moduleIcon = "<i style='font-size:20px;' class='ti-headphone-alt' title='$title'></i>";
		} else if ($title == 'Dashboard') {
			$moduleIcon = "<i style='font-size:20px;' class='ti-home' title='$title'></i>";
		}
		// else if($title == 'Task'){
		// 	$moduleIcon = "<i style='font-size:20px;' class='ti-check-box' title='$title'></i>";
		// }
