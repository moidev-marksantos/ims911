...
class Responderlogs extends Vtiger_CRMEntity {
	var $table_name = 'vtiger_responderlogs';
	var $table_index= 'responderlogsid';

	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('vtiger_responderlogscf', 'responderlogsid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('vtiger_crmentity', 'vtiger_responderlogs', 'vtiger_responderlogscf');
	...
	var $related_tables = Array( 
					'vtiger_responderlogscf' => Array('responderlogsid')
					);
	...
	var $tab_name_index = Array(
		...
		'vtiger_responderlogs'   => 'responderlogsid',
	    'vtiger_responderlogscf' => 'responderlogsid');
	...
	var $list_fields = Array (
		...
		'Responderlogs No' => Array('responderlogs', 'responderlogsno'),
/*'Responderlogs No'=> Array('responderlogs', 'responderlogsno'),*/
		'Assigned To' => Array('crmentity','smownerid')
	);
	var $list_fields_name = Array (
		/* Format: Field Label => fieldname */
		'Responderlogs No' => 'responderlogsno',
/*'Responderlogs No'=> 'responderlogsno',*/
		'Assigned To' => 'assigned_user_id'
	);

	// Make the field link to detail view
	var $list_link_field = 'responderlogsno';
...
/*'Responderlogs No'=> 'responderlogsno',*/
	...
	var $popup_fields = Array ('responderlogsno');

	// For Alphabetical search
	var $def_basicsearch_col = 'responderlogsno';

	// Column value to use on detail view record text display
	var $def_detailview_recname = 'responderlogsno';

	// Used when enabling/disabling the mandatory fields for the module.
	// Refers to vtiger_field.fieldname values.
	var $mandatory_fields = Array('responderlogsno','assigned_user_id');

	var $default_order_by = 'responderlogsno';
	...
	*/
	function vtlib_handler($moduleName, $eventType) {
		global $adb;
 		if($eventType == 'module.postinstall') {
			...
			Responderlogs::checkWebServiceEntry();
			Responderlogs::createUserFieldTable($moduleName);
			Responderlogs::addInTabMenu($moduleName);
	...
	static function checkWebServiceEntry() {
		...
		$sql       =  "SELECT count(id) AS cnt FROM vtiger_ws_entity WHERE name = 'Responderlogs'";
		...
		if($adb->num_rows($result) > 0)
		{
			...
			if($no == 0)
			{
				...
				$ws_entitySql = "INSERT INTO vtiger_ws_entity ( id, name, handler_path, handler_class, ismodule ) VALUES".
						  " (?, 'Responderlogs','include/Webservices/VtigerModuleOperation.php', 'VtigerModuleOperation' , 1)";
